...
<script setup lang="ts" name="MonitorCacheIndex">
import { onMounted, ref } from 'vue'
import { useCacheInfoApi } from '@/api/monitor/cache'
import type { RedisInfo } from '@/types/api/monitor/cache'

const data = ref<RedisInfo>({
  info: {},
  keyCount: 0,
  commandStats: []
})
...
function getCacheInfo() {
  useCacheInfoApi().then(res => {
    data.value = res.data || { info: {}, keyCount: 0, commandStats: [] }
  })
}
