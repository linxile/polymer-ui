...
<script setup lang="ts" name="MonitorServerIndex">
...
import type { Server } from '@/types/api/monitor/server'

const data = ref<Server>({})

/** 获取服务器聚合信息 */
function getServerInfo() {
  useServerInfoApi().then(res => {
    data.value = res.data || {}
  })
}
...
function refreshCpuInfo() {
  useCpuInfoApi().then(res => {
    data.value.cpu = res.data
  })
}

/** 刷新内存信息 */
function refreshMemInfo() {
  useMemInfoApi().then(res => {
    data.value.mem = res.data
  })
}

/** 刷新 JVM 信息 */
function refreshJvmInfo() {
  useJvmInfoApi().then(res => {
    data.value.jvm = res.data
  })
}

/** 刷新系统信息 */
function refreshSysInfo() {
  useSysInfoApi().then(res => {
    data.value.sys = res.data
  })
}

/** 刷新磁盘信息 */
function refreshDiskInfo() {
  useDiskInfoApi().then(res => {
    data.value.disks = res.data
  })
}
